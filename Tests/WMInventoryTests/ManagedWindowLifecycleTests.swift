...
final class ManagedWindowLifecycleTests: XCTestCase {
    func testTransientOmissionRetainsWindowUntilSuccessfulAppScanConfirmsClosure() {
        ...
        XCTAssertEqual(lifecycle.reconcile(snapshot(windows: [window("a", pid: 7)], pids: [7])).windows.map(\.id), ["a"])
    ...
    func testFailedTopLevelApplicationEnumerationCannotConfirmClosure() {
        ...
        let failed = lifecycle.reconcile(snapshot(windows: [], pids: [], accessibilityStatus: .unhealthy))
    ...
    func testPIDRestartEvictsEveryOldWindowAndInsertsReplacementFresh() {
        var lifecycle = ManagedWindowLifecycle()
        _ = lifecycle.reconcile(snapshot(windows: [window("a", pid: 7), window("b", pid: 7)], pids: [7]))
        _ = lifecycle.setOverride(.unmanaged, for: "a")

        let restarted = lifecycle.reconcile(snapshot(windows: [window("a", pid: 8), window("c", pid: 8)], pids: [8]))

        XCTAssertEqual(restarted.verifiedClosedLifetimes, [
    ...
    func testManageAndUnmanageOverridesLastForLogicalWindowLifetime() {
        var lifecycle = ManagedWindowLifecycle()
        _ = lifecycle.reconcile(snapshot(windows: [window("normal", pid: 7), window("dialog", pid: 7, classification: .transient)], pids: [7]))
        _ = lifecycle.setOverride(.unmanaged, for: "normal")
        _ = lifecycle.setOverride(.managed, for: "dialog")

        let updated = lifecycle.reconcile(snapshot(windows: [window("normal", pid: 7), window("dialog", pid: 7, classification: .transient)], pids: [7]))
        XCTAssertEqual(updated.windows.map(\.id), ["dialog"])

        _ = lifecycle.reconcile(snapshot(windows: [], pids: [7]))
        let replacements = lifecycle.reconcile(snapshot(windows: [window("normal", pid: 7), window("dialog", pid: 7, classification: .transient)], pids: [7]))
...
private func snapshot(
    ...
    InventorySnapshot(
        timestamp: .distantPast, durationMilliseconds: 0, displays: [], rawAXWindows: [], rawCGWindows: [],
        windows: windows, rejectedAXWindows: [], joinDecisions: [],
        sourceHealth: [.init(source: .accessibility, status: accessibilityStatus, permissionGranted: accessibilityStatus == .healthy)],
        appScans: pids.map { pid in
            .init(application: .init(pid: pid, name: "App"), status: status, durationMilliseconds: 0, windowCount: windows.filter { $0.pid == pid }.count, issues: [])
        }
    )
}

private func window(_ id: String, pid: Int32, classification: WindowClassification = .normal) -> NormalizedWindow {
    .init(
        ...
        management: .unmanaged, rejectionReasons: [], joinConfidence: .exact, joinSignals: [], health: .healthy,
        healthIssues: []
    )
}
