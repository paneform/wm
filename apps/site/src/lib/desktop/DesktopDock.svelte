...
<style>
  ...
  .screen-dock { position: absolute; z-index: var(--layer-screen-dock); inset: auto 50% var(--dock-screen-bottom) auto; width: max-content; max-width: 100%; transform: translateX(50%); backface-visibility: hidden; will-change: transform; padding: var(--dock-padding); border: var(--stroke-hairline) solid var(--color-line-default); border-radius: var(--radius-control); background: var(--color-window-background); pointer-events: auto; }
  button { position: relative; display: grid; width: var(--dock-icon-size); max-width: 100%; min-width: 0; min-height: 0; aspect-ratio: 1; place-items: center; border: 0; border-radius: var(--radius-small); padding: 0; background: var(--dock-icon-background); box-shadow: inset 0 var(--stroke-hairline) var(--space-1) color-mix(in srgb, var(--color-page-foreground) 8%, transparent), 0 var(--stroke-hairline) var(--space-2) color-mix(in srgb, var(--rp-base) 24%, transparent); color: var(--color-page-foreground); transition: background var(--motion-feedback) var(--easing-standard), box-shadow var(--motion-feedback) var(--easing-standard), transform var(--motion-feedback) var(--easing-standard); }
