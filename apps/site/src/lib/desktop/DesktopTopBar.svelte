...
<style>
  ...
  button { --workspace-color: var(--rp-subtle); --workspace-item-height: max(0px, calc(100cqh - 2 * var(--stroke-hairline))); display: flex; min-width: 0; align-items: center; gap: min(0.25rem, 12cqh); overflow: hidden; border: var(--stroke-hairline) solid var(--workspace-color); border-radius: min(0.25rem, 16cqh); padding: 0 min(0.3rem, 15cqh) 0 0; background: color-mix(in srgb, var(--rp-surface) 38%, transparent); color: var(--workspace-color); font: inherit; font-size: min(0.8rem, calc(var(--workspace-item-height) * 0.86)); line-height: 1; white-space: nowrap; transition: border-color var(--motion-feedback) var(--easing-standard), background var(--motion-feedback) var(--easing-standard), color var(--motion-feedback) var(--easing-standard); }
  ...
  strong { display: grid; align-self: stretch; min-width: 1.55em; flex: none; place-items: center; border-radius: min(0.2rem, 14cqh); background: var(--workspace-color); color: var(--rp-base); }
