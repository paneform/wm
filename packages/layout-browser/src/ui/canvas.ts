import type { BspTreeSnapshot, DisplayObservation, Frame, StateSnapshot } from "@paneform/layout";
...
// committed-state epoch changes (see host.ts subscription).
...
export type WindowVisualState = "managed" | "floating" | "parked" | "quarantined" | "ignored";
...
export function splitLinesForTree(
  ...
  const walk = (node: BspTreeSnapshot, rect: Frame): void => {
    ...
    if (node.axis === "vertical") {
      ...
      walk(node.second, {
        ...rect,
        y: rect.y + firstLen + gap,
        height: Math.max(0, rect.height - firstLen - gap),
      });
    }
  };
  walk(tree, content);
...
export function buildScene(snapshot: StateSnapshot, extras: SceneExtras = {}): Scene {
...
export function hitTestWindow(scene: Scene, worldPoint: ScreenPoint): string | null {
  const hits = scene.windows.filter(
    (w) =>
