...
// projection — see the integration seam notes in host.ts.
...
export function createPanels(deps: PanelDeps): PanelHandles {
  ...
  function refreshReplayOptions(): void {
    ...
    replaySelect.replaceChildren(...names.map((name) => el("option", { value: name, text: name })));
  }
  ...
  root.append(inspector, logPanel);
  if (deps.showCommands) root.append(commands);
  if (deps.showScenarios) {
    root.append(scenariosPanel);
    refreshReplayOptions();
  }
  ...
  return {
    ...
    populateSelects(snapshot) {
      ...
      syncSelect(
        windowSelect,
        snapshot.windows.map((w) => [w.id, labelFor(w)]),
      );
      syncSelect(
        displaySelect,
        snapshot.topology.map((d) => [d.id, d.id.replace("display:sim-", "")]),
      );
    ...
    logEvent(event) {
      ...
      const head = el("span", {
        class: `log-topic t-${event.topic}`,
        text: `[${event.seq}] ${event.topic}`,
      });
...
function renderInspector(
  ...
  if (groundTruth !== null) {
    const displays = groundTruth.displays
      .map(
        (d) =>
          `${d.id.replace("display:sim-", "")} ${fmtFrame(d.frame)}${d.primary ? " primary" : ""}`,
      )
      .join("\n");
    frag.append(
      el("pre", {
