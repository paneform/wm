...
/** Scenario context mirroring playground.ts wiring (never throws). */
...
describe("web platform sim determinism", () => {
  const scriptedOps = async (sim: WebPlatformSim) => {
    ...
    const results = [];
    results.push(
      await Effect.runPromise(adapter.setWindowSize(clamper, { width: 500, height: 400 })),
    );
    results.push(
      await Effect.runPromise(
        adapter.setWindowFrame(normal, { x: -2600, y: -2200, width: 500, height: 400 }),
      ),
    );
    // Fixed-size window refuses size writes at the API level.
    results.push(
      await Effect.runPromise(
        Effect.either(adapter.setWindowSize(fixed, { width: 200, height: 200 })),
      ),
    );
    results.push(
      await Effect.runPromise(adapter.setWindowPosition(normal, { x: -5000, y: -4000 })),
    );
  ...
  it("engine-level: identical seeds yield identical committed snapshots and event traces", async () => {
    const boot = async (): Promise<Harness> => {
      ...
      await harness.execute({
        ...
        windowId: harness.sim.windowIds()[0]!,
        workspace: "2",
      });
...
describe("sim personalities", () => {
  ...
  it("work-area clamping pulls workAreaClamp personalities back inside the usable area", () => {
    ...
    const clamped = clampFrameIntoWorkArea({ x: 1400, y: 500, width: 400, height: 300 }, workAreas);
  ...
  it("reanchoring apps shift origin on size-only writes (center stays)", async () => {
    ...
    const resized = await Effect.runPromise(
      sim.adapter.setWindowSize(id, { width: 200, height: 100 }),
    );
  ...
  it("animated settling advances per settle-read and snaps within a point", async () => {
    ...
    const mid = stepTowardFrame(
      { x: 0, y: 0, width: 10, height: 10 },
      { x: 90, y: 90, width: 10, height: 10 },
      0.5,
    );
    expect(mid.x).toBe(45);
    const near = stepTowardFrame(
      { x: 89.5, y: 0, width: 10, height: 10 },
...
describe("engine boot integration", () => {
  /**
   * Mirrors playground.ts bootstrap: seed membership through moveWindowToWorkspace
   ...
   * host.ts for why command receipts alone don't converge the projection.
   */
  it("tiles all normal windows onto the primary work area, disjoint panes", async () => {
    ...
    for (const id of seeded) {
      await Effect.runPromise(
        Effect.either(
          engine.execute({ type: "moveWindowToWorkspace", windowId: id, workspace: "1" }),
        ),
      );
    }
