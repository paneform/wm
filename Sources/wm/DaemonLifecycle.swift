...
actor SessionTransitionEpochs<Display: Equatable & Sendable, Inventory: Sendable> {
  ...
  func activationCause() -> SessionTransitionCause {
    pendingCause == .sleep || pendingCause == .wake || pendingCause == .lock
      ? .unlock : .activeSession
  }
  ...
  private func runPending(_ cause: SessionTransitionCause, run: Run) async throws
...
struct StartupIntentAudit {
  static func candidate(
    ...
    replacements: [String: String] = [:], pruneMissing: Bool = true
  ) -> WMWorkspace.WorkspaceState {
    guard inventory.sourceHealth.first(where: { $0.source == .coreGraphics })?.status == .healthy
    else {
      return state
    }
    ...
    func isDefinitivelyAbsentAXIdentity(_ id: String) -> Bool {
      guard id.hasPrefix("window:ax:"),
        let pid = Int32(id.split(separator: ":", maxSplits: 3)[2])
      else { return false }
...
extension WorkspaceIntentAudit {
  var orderedSteps: [WorkspaceIntentAuditStep] {
    restore.keys.sorted().compactMap { id in
      restore[id].map { .init(windowOrWorkspaceID: id, action: .restore($0)) }
    }
...
extension ParkedWindowFrame {
  fileprivate var inventoryRect: InventoryRect { .init(x: x, y: y, width: width, height: height) }
}
